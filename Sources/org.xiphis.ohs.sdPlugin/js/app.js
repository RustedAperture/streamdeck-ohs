...
function connected(jsn) {
  ...
  $SD.on("org.xiphis.ohs.action.willAppear", (jsonObj) =>
    action.onWillAppear(jsonObj)
  );
  $SD.on("org.xiphis.ohs.action.keyUp", (jsonObj) => action.onKeyUp(jsonObj));
  $SD.on("org.xiphis.ohs.action.sendToPlugin", (jsonObj) =>
    action.onSendToPlugin(jsonObj)
  );
  $SD.on("org.xiphis.ohs.action.didReceiveSettings", (jsonObj) =>
...
const action = {
  ...
  onDidReceiveSettings: function (jsn) {
    ...
    if (settings.hasOwnProperty("sensor_fontsize")) {
      sensor.setFontSize(settings.sensor_fontsize);
    }
  },
...
function OpenHardwareSensor(jsonObj) {
  ...
  function updateLine(doFill) {
    ...
    const fontSize = getFontSize() * smaller;
    ...
    let w = [];
    for (
      var wx = 0, wd = (width - 1) / (max_history + 0.1);
      wx <= width;
      wx += wd
    ) {
      w.push(wx);
  ...
  function updateKnob() {
    ...
    knob.setProperty("fontsize", getFontSize());
